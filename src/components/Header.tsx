...
// Hoofdmenu: precies vijf pagina's, altijd zichtbaar bovenaan.
const navItems = [
  { href: "/", label: "Home" },
  ...
  return (
    <header className="sticky top-0 z-40 border-b border-brand-100 bg-white/90 backdrop-blur supports-[backdrop-filter]:bg-white/75">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="flex h-16 items-center justify-between gap-4">
          <Link
            ...
            aria-label="Kanopolo Community home"
          >
            <Waves className="h-7 w-7 text-brand-500" aria-hidden="true" />
            <span className="text-lg font-bold tracking-tight">
              Kanopolo<span className="text-accent-500">Community</span>
            </span>
          </Link>
      ...
      {open && (
        ...
        >
          <nav aria-label="Mobiele navigatie" className="grid gap-1 px-4 py-4">
            {navItems.map((item) => (
              <Link
                key={item.href}
                href={item.href}
